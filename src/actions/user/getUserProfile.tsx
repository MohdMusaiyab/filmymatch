...
export async function getUserProfile({
    ...
    try {
        ...
        const signedPosts = await enhancePostsWithSignedUrls(
            posts.map((post) => ({
                ...post,
                images: [], // Add an empty images array to satisfy the PostWithImages type
            }))
        ).then((enhancedPosts) =>
            enhancedPosts.map((post, index) => ({
                ...posts[index], // Preserve original properties
                coverImage: post.coverImage, // Update with signed URL if modified
            }))
        );
            ...
            data: {
                ...
                highlights: {
                    items: highlights.map((highlight, index) => ({
                        ...
                        post: {
                            ...highlight.post,
                            coverImage: signedHighlights[index].coverImage, // Update with signed URL
                        },
                    })),
